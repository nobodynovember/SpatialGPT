...
class SpatialExpert:
    ...
    def save_spatial_observation(self, obs, t, env):
        ...
        # print('current distance:', estimate_distance)
    ...
    #  parse the instruction into a sequence of actions and landmarks with spatial area
        ...
        if (instru_step==len(self.actionlist)-1): # last action
            prompt_system, prompt_user=self.prompt_manager.make_gpt_stop_reason_prompt(ob, current_action_name, navigable_images)
            # prompt_system, prompt_user=self.prompt_manager.make_gpt_stop_reason_prompt(ob, self.extracted_instruction, navigable_images)
        else:
            prompt_system, prompt_user=self.prompt_manager.make_gpt_breadth_reason_prompt(ob, self.extracted_instruction,orientation, navigable_images, current_action_name)
        # print('search landmarks prompt:', prompt_system, prompt_user)
        image_list = []
        temp_list=[]
        for ix in range(12):
        ...
        if t>=len(self.actionlist)-1:  # after number of actions
            ...
            self.prev_distance=estimated_distance

            if float(estimated_distance)<-0.5:
                print('Destination Invisible at Additional Distance Estimation')
            else: 
                print('Destination Seen at Additional Distance Estimation (Meter):', estimated_distance)          
                if float(estimated_distance)<1.6: 
                    self.stopping=True

        self.stop_distance=reason_result['stop_distance']
        if float(self.stop_distance)<-0.5:
            ...
            if float(self.stop_distance)<1.6: 
                self.stopping=True
            
        # if self.check_down_elevation==True and (self.current_instru_step in int_down_steps):
        #     print('down stop distance:', down_reason_result['stop_distance'])
        #     if float(down_reason_result['stop_distance'])<-0.5: # detination invisible
        #         print('down destination invisible')
        #     else:
        #         if float(down_reason_result['stop_distance'])<1: # stop when downstairs               
        #             if bool(self.args.stop_after):
        #                 if t >= self.args.stop_after:
        #                     self.stop_flag=True
        #                     print('GPT return STOP')
